...
async def get_thumb(videoid, user_id, user_name):
    ...
    final_path = f"cache/{videoid}_{user_id}_v2.png"
    ...
    try:
        ...
        # Rain overlay: deterministic per track, subtle enough to keep the
        # title and controls readable on Telegram's small media preview.
        rain = Image.new("RGBA", background.size, (0, 0, 0, 0))
        rain_draw = ImageDraw.Draw(rain, "RGBA")
        rain_rng = random.Random(str(videoid))
        for _ in range(115):
            x = rain_rng.randint(-100, 1920)
            y = rain_rng.randint(0, 1080)
            length = rain_rng.randint(12, 34)
            rain_draw.line(
                (x, y, x - 10, y + length),
                fill=(180, 225, 255, rain_rng.randint(28, 72)),
                width=rain_rng.choice((1, 1, 2)),
            )
        ...
        shine_draw.polygon(
            [(250, 70), (520, 70), (1120, 900), (900, 900)],
            fill=(255, 255, 255, 16),
        )
        shine_draw.ellipse((1450, 105, 1510, 165), fill=(255, 255, 255, 80))
        shine_draw.ellipse((1520, 175, 1545, 200), fill=(255, 255, 255, 55))
